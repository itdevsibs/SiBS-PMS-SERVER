  ...
  const [rows] = await pmsDb.query(
    `
      SELECT
        ...
        SUM(CASE WHEN ${answeredExpression} AND a.handle_seconds IS NOT NULL THEN a.handle_seconds ELSE 0 END) AS handle_seconds_total,
        SUM(CASE WHEN ${answeredExpression} AND a.handle_seconds IS NOT NULL THEN 1 ELSE 0 END) AS handle_seconds_count,
